...
def _tick_unified_radar(
    ...
    *,
    ...
    if runtime is None:
        return None
    runtime.set_watched_pairs(set(_mexc_watchlist(state_mgr)))
    snapshot = runtime.tick(force=force)
    if snapshot is None:
    ...
    if previous.get("generated_at") != payload.get("generated_at"):
        ...
        _sync_mexc_watchlist_state(payload, state_mgr)
        state_mgr.save()
    return payload
...
PM_CORE_TP1_R = env_float("PM_CORE_TP1_R", 0.6, min_value=0.1)
PM_CORE_TP2_R = env_float("PM_CORE_TP2_R", 2.2, min_value=0.2)
PM_CORE_TP3_R = env_float("PM_CORE_TP3_R", 5.0, min_value=0.3)
PM_HIGH_BETA_TP1_R = env_float("PM_HIGH_BETA_TP1_R", 0.6, min_value=0.1)
...
PM_TRAIL_STEPS_CORE = [
    (0.6, 0.00),   # TP1 0.6R hit -> stop breakeven (TP1 ile senkron)
    (1.5, 0.60),   # +1.5R -> +0.60R lock
    (2.5, 1.50),   # +2.5R -> +1.50R lock
    (4.0, 3.00),   # +4.0R -> +3.00R lock
]
PM_TRAIL_STEPS_HIGH_BETA = [
    (0.6, 0.00),   # TP1 0.6R hit -> BE
    (1.4, 0.55),
    (2.2, 1.30),
    (3.5, 2.50),
]
...
REGIME_STRATEGY_MATRIX: dict[str, dict[str, Any]] = {
    ...
    "RISK_OFF_TREND_DOWN": {
        ...
        "quality_bonus": -10,
        "tp_style": "cash",
        "note": "Risk-off düşüş: SHORT yönünün 2022-07/2023-10 backtestlerinde PF<0.6 oldugu icin yeni trade kapali; nakit kal.",
    },
    "CHOP_RANGE": {
        "direction_bias": "LONG_ONLY_TACTICAL",
        "allow_long": True,
        "allow_short": False,
        "high_beta_allowed": False,
        ...
        "quality_bonus": -5,
        "tp_style": "quick_tp",
        "note": "Chop/range: taktiksel scalp; SHORT yonu disabled (backtest PF<0.6), sadece long, KUCUK risk (0.30x), hizli TP.",
    },
    "NEWS_CHAOS": {
    ...
    "SQUEEZE_SHORT": {
        ...
        "quality_bonus": -10,
        "tp_style": "cash",
        "note": "Long liquidation ihtimali: SHORT yonu backtestlerde kaybediyor (PF<0.6), yeni trade kapali.",
    },
    "NEUTRAL": {
        "direction_bias": "NO_NEW_TRADE",
        "allow_long": False,
        ...
        "quality_bonus": -10,
        "tp_style": "cash",
        "note": "Notr rejim: 2022-07 (PF 0.68) ve 2023-10 (PF 0.85) backtestlerinde kaybediyor; yeni trade kapali, nakit kal.",
    },
}
...
_KEYBOARD_COMMAND_LABELS: dict[str, str] = {
    ...
    "👀 Takip": "WATCHLIST",
    "🎯 Eşiği Geçen": "LISTINGS",
    "🟡 Filtredekiler": "FILTERED_LISTINGS",
    "👀 Takip Listem": "WATCHLIST",
    "🔄 Şimdi Tara": "MEXC_REFRESH",
    # Command Center v2
    "🆕 MEXC": "LISTINGS",
...
def _extract_command_text(update: dict) -> tuple[Optional[str], Optional[str]]:
    ...
    message = (
        ...
        else update.get("message") or update.get("edited_message") or {}
    )
...
def _answer_telegram_callback(update: dict) -> None:
    callback = update.get("callback_query") or {}
    ...
    try:
        with _HTTP_SEMAPHORE:
            _http_session().post(
                f"{TELEGRAM_BASE}/bot{TOKEN}/answerCallbackQuery",
...
_MEXC_WATCH_PAIR_RE = re.compile(r"^[A-Z0-9]{2,40}USDT$")
_MEXC_WATCHLIST_META_KEY = "mexc_manual_watchlist"


def _normalize_mexc_watch_pair(value: Any) -> Optional[str]:
    pair = str(value or "").strip().upper().replace("/", "").replace("-", "")
    if pair and not pair.endswith("USDT"):
        pair = f"{pair}USDT"
    return pair if _MEXC_WATCH_PAIR_RE.fullmatch(pair) else None


def _mexc_watchlist(state_mgr: StateManager = _STATE_MGR) -> dict[str, dict]:
    payload = state_mgr.get_meta(_MEXC_WATCHLIST_META_KEY, {})
    if not isinstance(payload, dict):
        return {}
    return {
        pair: copy.deepcopy(row)
        for pair, row in payload.items()
        if _normalize_mexc_watch_pair(pair) == pair and isinstance(row, dict)
    }


def _save_mexc_watchlist(watchlist: dict[str, dict], state_mgr: StateManager) -> None:
    state_mgr.set_meta(_MEXC_WATCHLIST_META_KEY, copy.deepcopy(watchlist))
    state_mgr.save()


def _listing_candidate_rows(snapshot: Any) -> list[dict]:
    if snapshot is None:
        return []
    ...
    if not isinstance(data, dict):
        return []
    rows = [
        *(data.get("listing_candidates") or []),
        *(data.get("listing_filtered_candidates") or []),
    ]
    return [row for row in rows if isinstance(row, dict)]


def _mexc_candidate_by_pair(snapshot: Any) -> dict[str, dict]:
    return {
        str(row.get("symbol") or "").upper(): row
        for row in _listing_candidate_rows(snapshot)
...
def add_mexc_watch(
    ...
    watchlist[pair] = row
    _save_mexc_watchlist(watchlist, state_mgr)
    runtime = _get_unified_runtime()
    if runtime is not None:
        runtime.set_watched_pairs(set(watchlist))
        runtime.tick(force=True)
    if already:
        return True, f"👀 {pair} zaten takip listende."
    return True, (
        f"👀 {pair} takibe alındı. Filtreyi geçmese bile yeniden puanlanacak; "
        "ısındığında haber vereceğim. Bu işlem emir yetkisi vermez."
    )


def remove_mexc_watch(
    value: Any,
    state_mgr: StateManager = _STATE_MGR,
) -> tuple[bool, str]:
    pair = _normalize_mexc_watch_pair(value)
    if not pair:
        return False, "Geçersiz coin. Örnek: /watch_remove NOVA"
    watchlist = _mexc_watchlist(state_mgr)
    if pair not in watchlist:
        return False, f"{pair} takip listende değil."
    watchlist.pop(pair, None)
    _save_mexc_watchlist(watchlist, state_mgr)
    runtime = _get_unified_runtime()
    if runtime is not None:
        runtime.set_watched_pairs(set(watchlist))
    return True, f"🗑 {pair} takip listesinden çıkarıldı."


def _sync_mexc_watchlist_state(snapshot: dict, state_mgr: StateManager) -> None:
    watchlist = _mexc_watchlist(state_mgr)
    if not watchlist:
        return
    ...
    for pair, entry in watchlist.items():
        ...
        if not current:
            continue
        previous = entry.get("last_candidate")
        old_metadata = previous.get("metadata") if isinstance(previous, dict) else {}
        new_metadata = current.get("metadata") or {}
        old_status = str((old_metadata or {}).get("filter_status") or "")
    ...
    state_mgr.set_meta(_MEXC_WATCHLIST_META_KEY, watchlist)
...
def _friendly_listing_stage(value: Any) -> str:
    return {
        "HOT": "🔥 Güçlü",
        "BUILDING": "📈 Isınıyor",
        "WATCH": "👀 İzle",
...
def _format_mexc_watchlist(state_mgr: StateManager) -> str:
    watchlist = _mexc_watchlist(state_mgr)
    if not watchlist:
        return (
            "👀 MEXC TAKİP LİSTEM\n\nTakipte coin yok. "
    ...
    for index, (pair, entry) in enumerate(ordered, 1):
        candidate = entry.get("last_candidate") or {}
        metadata = candidate.get("metadata") or {}
        if candidate:
            lines.extend(
                [
                    f"{index}. {pair} — {int(candidate.get('score') or 0)}/100 · {_friendly_listing_stage(candidate.get('stage'))}",
                    f"   5dk ivme {float(metadata.get('volume_acceleration') or 0):.1f}x | değişim %{float(metadata.get('change_pct') or 0):+.1f}",
                ]
            )
            filter_reasons = metadata.get("filter_reasons") or []
            if filter_reasons:
                lines.append("   Bekliyor: " + " · ".join(str(x) for x in filter_reasons[:2]))
        else:
            lines.append(f"{index}. {pair} — ilk MEXC teyidi bekleniyor")
        lines.append("")
    lines.append("Takip listesi işlem evrenini değiştirmez ve otomatik emir vermez.")
    return "\n".join(lines)


def _mexc_watch_status(value: Any, state_mgr: StateManager) -> str:
    pair = _normalize_mexc_watch_pair(value)
    if not pair:
        return "Geçersiz coin."
    entry = _mexc_watchlist(state_mgr).get(pair)
    if not entry:
        return f"{pair} takip listende değil."
    current = _mexc_candidate_by_pair(
        state_mgr.get_meta("unified_radar_snapshot")
    ).get(pair) or entry.get("last_candidate") or {}
    if not current:
        return f"✅ {pair} takipte. İlk MEXC teyidi bekleniyor."
    metadata = current.get("metadata") or {}
    filter_status = str(metadata.get("filter_status") or "").upper()
    filter_text = "Eşiği geçti" if filter_status == "PASSED" else "Filtrede izleniyor"
    try:
        acceleration = float(metadata.get("volume_acceleration") or 0)
        change = float(metadata.get("change_pct") or 0)
    except (TypeError, ValueError):
        acceleration, change = 0.0, 0.0
    return (
        f"✅ MEXC TAKİP DURUMU\n\n"
        f"{pair} — {int(current.get('score') or 0)}/100 · "
        f"{_friendly_listing_stage(current.get('stage'))}\n"
        f"Durum: {filter_text}\n"
        f"5dk hacim ivmesi: {acceleration:.1f}x\n"
        f"24s değişim: %{change:+.1f}\n\n"
        "Takip alarmı aktiftir; otomatik emir oluşturmaz."
    )
...
def _process_telegram_commands_locked(state_mgr: StateManager = _STATE_MGR) -> None:
    ...
    for update in updates:
        ...
        if not text:
            continue
        _answer_telegram_callback(update)
...
def telegram_decision_fingerprint(
    state_mgr: StateManager,
    results: list[dict],
    session_ctx: SessionContext,
    news_ctx: dict,
) -> str:
    """Stable fingerprint for Telegram decision-change filtering.

    The goal is to avoid messages for small numeric score changes. We only
    notify when the practical decision state changes:
      - WAIT / LONG SETUP WATCH / BLOCKED / MANAGE POSITIONS
      - regime bucket
      - macro bucket
      - risk mode
      - open position count
      - best candidate symbol if any
      - news category
    """
    ...
    regime_bucket = format_regime_strategy_brief(results).replace("Regime: ", "").split("|")[0].strip()
    macro_bucket = format_macro_risk_brief(state_mgr).replace("MacroRisk: ", "").split("|")[0].strip()

    best_symbol = "-"
    best_signal = "-"
    candidates = [
        r for r in results
        if r.get("signal") in {"LONG", "SHORT"} and r.get("actionable", True)
    ]
    if candidates:
        best = sorted(candidates, key=lambda x: float(x.get("confidence", 0) or 0), reverse=True)[0]
        best_symbol = str(best.get("symbol") or "-")
        best_signal = str(best.get("signal") or "-")

    payload = {
        "decision": decision.get("decision"),
        "trade_allowed": bool(decision.get("trade_allowed")),
        "regime": regime_bucket,
        "macro": macro_bucket,
        "risk_mode": str(rg.get("mode", "-")),
...
def should_send_decision_report(
    state_mgr: StateManager,
    results: list[dict],
    session_ctx: SessionContext,
    news_ctx: dict,
) -> tuple[bool, str, str]:
    """Return whether heartbeat should be sent.

    If TELEGRAM_DECISION_CHANGE_ONLY=1, send only when the decision fingerprint
    changes. Optionally force a heartbeat every TELEGRAM_FORCE_HEARTBEAT_SECONDS.
    """
    current = telegram_decision_fingerprint(state_mgr, results, session_ctx, news_ctx)
    ...
    if TELEGRAM_FORCE_HEARTBEAT_SECONDS > 0 and now_ts() - last_sent >= TELEGRAM_FORCE_HEARTBEAT_SECONDS:
        return True, current, "forced_heartbeat"
...
def unified_radar_loop(
    ...
    poll_seconds = 5
    log.info(
        "Unified radar listener başladı (tarama aralığı %ss).",
    ...
    while not stop_event.is_set():
        try:
            ...
            log.warning("Unified radar listener hatası: %s", exc)
