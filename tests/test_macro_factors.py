...
def _core_history():
    rows = []
    levels = {"us_2y":4.0,"us_10y":4.5,"us_30y":5.0,"us_10y_real":2.0,"fed_assets":7_000_000.0,"rrp":100.0,"tga":800_000.0,"broad_usd":120.0,"usdjpy":145.0}
    increments = {"us_2y":0.01,"us_10y":0.02,"us_30y":0.03,"us_10y_real":0.01,"fed_assets":10_000.0,"rrp":-5.0,"tga":20_000.0,"broad_usd":0.5,"usdjpy":1.0}
...
def test_factor_snapshot_does_not_pretend_insufficient_history_is_neutral():
    rows=[_obs(name,19,value) for name,value in {"us_2y":4.0,"us_10y":4.5,"us_30y":5.0,"us_10y_real":2.0,"fed_assets":7_000_000.0,"rrp":100.0,"tga":800_000.0,"broad_usd":120.0,"usdjpy":145.0}.items()]
    snapshot=build_factor_snapshot(rows,as_of=datetime(2026,8,20,18,tzinfo=timezone.utc))
    assert snapshot.complete is False; assert "us_10y_d1_bp" in snapshot.insufficient_history_factors; assert snapshot.factors["us_10y_d1_bp"] is None


def test_missing_fx_series_is_explicit_not_neutral():
    rows=[row for row in _core_history() if row.series!="usdjpy"]
    snapshot=build_factor_snapshot(rows,as_of=datetime(2026,8,20,18,tzinfo=timezone.utc))
    assert snapshot.complete is False; assert "usdjpy" in snapshot.missing_required_series; assert snapshot.factors["usdjpy_d1_pct"] is None


def test_optional_boj_gap_does_not_block_core_factor_completeness():
    snapshot=build_factor_snapshot(_core_history(),as_of=datetime(2026,8,20,18,tzinfo=timezone.utc))
    assert snapshot.factors["boj_call_rate_d1_bp"] is None; assert snapshot.complete is True
