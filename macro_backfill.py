...
# Research-only extensions. These are intentionally not added to the live
# collector in this change: historical evidence must be established before the
# new factors are allowed to become required live inputs.
RESEARCH_FRED_SERIES = {
    **FRED_SERIES,
    "broad_usd": "DTWEXBGS",  # Nominal Broad U.S. Dollar Index
    "usdjpy": "DEXJPUS",      # Japanese Yen to One U.S. Dollar
}
...
class FredRevisionBackfill:
    ...
    def fetch_series(self, logical_series: str, *, start: str | date | datetime, end: str | date | datetime, page_limit: int = 10000) -> tuple[MacroObservation, ...]:
        if not self.api_key:
            raise RuntimeError("FRED_API_KEY is required")
        if logical_series not in RESEARCH_FRED_SERIES:
            raise KeyError(f"unsupported research FRED series: {logical_series}")
        if page_limit < 1:
        ...
        series_id = RESEARCH_FRED_SERIES[logical_series]
        offset = 0
        parsed: list[MacroObservation] = []
        while True:
            params = {
                "series_id": series_id, "api_key": self.api_key, "file_type": "json",
                "output_type": 3, "sort_order": "asc",
                "realtime_start": start_day.isoformat(), "realtime_end": end_day.isoformat(),
                "observation_start": (start_day - timedelta(days=14)).isoformat(),
                "observation_end": end_day.isoformat(), "limit": page_limit, "offset": offset,
            }
            ...
            for row in rows:
                if row.get("value") in (None, "."):
                    continue
                vintage, observed = row.get("realtime_start"), row.get("date")
                if not vintage or not observed:
                    raise RuntimeError("FRED revision row missing date metadata")
                vintage_day, observed_day = date.fromisoformat(vintage), date.fromisoformat(observed)
                ...
                obs = MacroObservation(
                    provider="fred_alfred_reconstruction", series=logical_series, source_id=series_id,
                    observation_time=datetime.combine(observed_day, time.min, tzinfo=timezone.utc).isoformat(),
                    available_time=available.isoformat(), ingested_time=available.isoformat(), value=value,
                    realtime_start=vintage_day.isoformat(), realtime_end=row.get("realtime_end"),
                )
                obs.validate()
                parsed.append(obs)
            ...
            if not rows or offset >= count:
                break
        unique = {(row.series, row.observation_time, row.available_time, float(row.value)): row for row in parsed}
    ...
    def fetch_core(self, *, start: str | date | datetime, end: str | date | datetime) -> tuple[MacroObservation, ...]:
        ...
        for logical_series in RESEARCH_FRED_SERIES:
...
class BinanceHourlyBackfill:
    ...
    def __init__(self, session: requests.Session | None = None, config: BinanceBackfillConfig | None = None):
        ...
        if self.config.interval != "1h": raise ValueError("macro replay currently requires 1h bars")
        if not 1 <= self.config.limit <= 1000: raise ValueError("Binance limit must be in [1,1000]")
    ...
    def parse_rows(cls, rows: Iterable[list]) -> tuple[PriceBar, ...]:
        ...
        for row in rows:
            if len(row) < 7: raise RuntimeError("short Binance kline row")
            bar = PriceBar(timestamp=cls._iso_from_ms(int(row[6])), open=float(row[1]), high=float(row[2]), low=float(row[3]), close=float(row[4]))
            bar.validate(); parsed.append(bar)
        return tuple(parsed)

    def fetch(self, *, start: datetime, end: datetime) -> tuple[PriceBar, ...]:
        start, end = _utc(start), _utc(end)
        if end <= start: raise ValueError("price backfill end must follow start")
        cursor, end_ms = int(start.timestamp() * 1000), int(end.timestamp() * 1000)
        bars: list[PriceBar] = []
        while cursor < end_ms:
            response = self.session.get(self.url, params={"symbol": self.config.symbol, "interval": self.config.interval, "startTime": cursor, "endTime": end_ms, "limit": self.config.limit}, timeout=30)
            response.raise_for_status(); batch = self.parse_rows(response.json())
            if not batch: break
            bars.extend(batch)
            last_close_ms = int(datetime.fromisoformat(batch[-1].timestamp).timestamp() * 1000)
            next_cursor = last_close_ms + 1
            if next_cursor <= cursor: raise RuntimeError("Binance backfill cursor did not advance")
            cursor = next_cursor
            if len(batch) < self.config.limit: break
